...
export function MealCard({
  ...
  return (
    <div className="bg-card border border-border rounded-card overflow-hidden">
      <Link
        href={`/products/${slug}`}
        className={`block ${imageUrl ? 'aspect-video' : 'h-[130px]'} bg-gradient-to-br from-[var(--gradient-from)] to-[var(--gradient-to)] flex items-center justify-center text-5xl relative overflow-hidden`}
      >
        {imageUrl ? (
          <Image
            src={imageUrl}
